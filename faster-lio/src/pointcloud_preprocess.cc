...
void PointCloudPreprocess::Oust64Handler(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
    ...
    for (int i = 0; i < pl_orig.points.size(); i++) {
        ...
        // Check for NaN values in the point coordinates
        if (std::isnan(pl_orig.points[i].x) || std::isnan(pl_orig.points[i].y) || std::isnan(pl_orig.points[i].z) || std::isnan(pl_orig.points[i].t) || std::isnan(pl_orig.points[i].intensity)) {
            // RCLCPP_WARN(rclcpp::get_logger("PointCloudPreprocess"), "NaN value detected in point %d, skipping...", i);
            num_nan_points++;
            continue;
        }
